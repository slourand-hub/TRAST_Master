...
def compute_raw_diagnostics(files: List[str], max_h: int, default_duty_fraction: float = 0.01) -> pd.DataFrame:
    rows = []
    ...
    for path in files:
        ...
        for H in range(1, max_h + 1):
            ...
            row[f"sum{H}_abs_ls_raw"] = float(np.sum(ls_amps_raw[:H])) if np.all(np.isfinite(ls_amps_raw[:H])) else np.nan
